...
class VacationModal(discord.ui.Modal, title="🏖️ ЗАЯВКА НА ОТПУСК"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            print(f"📅 Дней: {days_num}, Причина: {self.reason.value}")
            ...
            # Сохраняем текущие роли пользователя с указанием сервера
            member = interaction.guild.get_member(interaction.user.id)
            saved_roles = [str(role.id) for role in member.roles if not role.is_default()]
            guild_id = str(interaction.guild.id)
            print(f"👥 Сохранено ролей с сервера {guild_id}: {len(saved_roles)}")
            ...
            if not applications_channel_id:
                print("❌ Канал заявок не настроен")
                return
            ...
            print(f"✅ Заявка на отпуск #{app_id} отправлена в канал {channel.name}")
