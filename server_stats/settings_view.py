...
class StatsSettingsView(AdminOnlyView):
    ...
    def _add_buttons(self):
        self.clear_items()
        channel_btn = discord.ui.Button(label="📊 Канал статистики", style=discord.ButtonStyle.primary, emoji="📊", row=0, custom_id="stats_channel")
        channel_btn.callback = self.set_stats_channel
        self.add_item(channel_btn)
        
        backup_btn = discord.ui.Button(label="💾 Бекап в ЛС", style=discord.ButtonStyle.primary, emoji="💾", row=0, custom_id="stats_backup")
        backup_btn.callback = self.toggle_backup
        self.add_item(backup_btn)
        
        show_btn = discord.ui.Button(label="📊 Текущие настройки", style=discord.ButtonStyle.secondary, emoji="📊", row=1, custom_id="stats_show")
    ...
    async def toggle_backup(self, interaction: discord.Interaction):
        if not await is_admin(str(interaction.user.id)):
            await interaction.response.send_message("❌ Только администраторы!", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"✅ Отправка бекапа в ЛС {'ВКЛЮЧЕНА' if new_value else 'ВЫКЛЮЧЕНА'}", ephemeral=True)

    async def show_settings(self, interaction: discord.Interaction):
        if not await is_admin(str(interaction.user.id)):
            await interaction.response.send_message("❌ Только администраторы!", ephemeral=True)
            return
        embed = discord.Embed(title="📊 НАСТРОЙКИ СТАТИСТИКИ СЕРВЕРА", color=0x00ff00)
...
class SetStatsChannelModal(discord.ui.Modal, title="📊 КАНАЛ СТАТИСТИКИ"):
    channel_id = discord.ui.TextInput(label="ID канала для статистики", placeholder="123456789012345678", max_length=20, required=True)
