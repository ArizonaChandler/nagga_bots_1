"""Панель управления CAPT с постоянными кнопками настроек"""
import discord
import logging
...
class CaptSettingsView(PermanentView):
    """Постоянные кнопки для настройки CAPT системы"""
    
    def __init__(self):
        super().__init__()
        logger.debug("CaptSettingsView создан")
    
    @discord.ui.button(
        label="🎯 Каналы регистрации", 
    ...
    async def set_channels(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Настроить каналы для регистрации"""
    ...
    async def set_alert_channel(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Настроить канал для оповещений"""
    ...
    async def show_settings(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="🎭 Роль для ЛС", value=role, inline=False)
        
        # Канал настроек
        settings_channel_id = CONFIG.get('capt_settings_channel')
        settings_channel = format_mention(guild, settings_channel_id, 'channel') if settings_channel_id else "`Не настроен`"
        embed.add_field(name="⚙️ Канал настроек", value=settings_channel, inline=False)
