...
class MoveToMainModal(discord.ui.Modal, title="➕ Добавить в основной список"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # СРАЗУ отвечаем, чтобы не было ошибки взаимодействия
        ...
        try:
            ...
            logger.error(f"Ошибка в MoveToMainModal: {e}", exc_info=True)
        ...
        try:
            ...
            if identifier.isdigit() and len(identifier) <= 3:
                ...
                if user_id:
                    ...
                    await interaction.followup.send(
                        f"❌ Номер {num} не найден в основном списке",
                        ephemeral=True
                    )
                    return
            ...
            logger.error(f"Ошибка в MoveToReserveModal: {e}", exc_info=True)
            await interaction.followup.send(f"❌ Ошибка: {e}", ephemeral=True)
