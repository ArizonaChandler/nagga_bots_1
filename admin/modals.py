...
class SetStatsSettingsChannelModal(discord.ui.Modal, title="📊 КАНАЛ НАСТРОЕК СТАТИСТИКИ"):
    def __init__(self, guild=None):
        super().__init__()
        self.guild = guild
    
    channel_id = discord.ui.TextInput(
        label="ID канала для настроек статистики",
        placeholder="ID канала где будут постоянные кнопки",
        max_length=20,
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            async for msg in channel.history(limit=50):
                if msg.author == interaction.client.user and msg.embeds:
                    if msg.embeds and "НАСТРОЙКИ СТАТИСТИКИ" in msg.embeds[0].title:
                        ...
                        message_exists = True
                        await interaction.response.send_message(
                            f"✅ Панель настроек статистики обновлена в {channel.mention}",
                            ephemeral=True
                        )
                        break
            ...
            if not message_exists:
                ...
                await interaction.response.send_message(
                    f"✅ Канал настроек статистики создан: {channel.mention}",
                    ephemeral=True
                )
...
class SetGlobalSettingsChannelModal(discord.ui.Modal, title="🎛️ КАНАЛ УПРАВЛЕНИЯ МОДУЛЯМИ"):
    channel_id = discord.ui.TextInput(label="ID канала", placeholder="123456789012345678", max_length=20, required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            print(f"❌ Ошибка в SetGlobalSettingsChannelModal: {e}")
            import traceback
            traceback.print_exc()
            await interaction.response.send_message(f"❌ Ошибка: {e}", ephemeral=True)