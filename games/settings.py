...
class GamesChannelsView(discord.ui.View):
    ...
    def __init__(self):
        ...
        rules_btn = discord.ui.Button(label="📜 Канал правил", style=discord.ButtonStyle.secondary, row=0)
        rules_btn.callback = self.set_rules_channel
        self.add_item(rules_btn)

        lobby_btn = discord.ui.Button(label="🎮 Канал лобби", style=discord.ButtonStyle.secondary, row=0)
        lobby_btn.callback = self.set_lobby_channel
        self.add_item(lobby_btn)

        log_btn = discord.ui.Button(label="📝 Канал логов", style=discord.ButtonStyle.secondary, row=1)
        log_btn.callback = self.set_log_channel
        self.add_item(log_btn)

        settings_btn = discord.ui.Button(label="⚙️ Канал настроек", style=discord.ButtonStyle.secondary, row=1)
        settings_btn.callback = self.set_settings_channel
        self.add_item(settings_btn)

        category_btn = discord.ui.Button(label="📁 Категория игр", style=discord.ButtonStyle.secondary, row=2)
        category_btn.callback = self.set_category
        self.add_item(category_btn)

        back_btn = discord.ui.Button(label="◀ Назад", style=discord.ButtonStyle.secondary, row=2)
