...
class TierSettingsView(PermanentView):
    """Постоянные кнопки для настройки системы TIR"""
...
class SetTierSubmitChannelModal(discord.ui.Modal, title="📝 КАНАЛ ПОДАЧИ ЗАЯВОК TIR"):
    def __init__(self):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                f"✅ Канал подачи заявок TIR настроен: {channel.mention}",
                ephemeral=True
            )
...
class SetTierApplicationsChannelModal(discord.ui.Modal, title="📋 КАНАЛ АНКЕТ TIR"):
    def __init__(self):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                f"✅ Канал анкет TIR настроен: {channel.mention}",
                ephemeral=True
            )
        except Exception as e:
            await interaction.response.send_message(f"❌ Ошибка: {e}", ephemeral=True)
...
class SetTierLogChannelModal(discord.ui.Modal, title="📜 КАНАЛ ЛОГОВ TIR"):
    def __init__(self):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                f"✅ Канал логов TIR настроен: {channel.mention}",
                ephemeral=True
            )
        except Exception as e:
            await interaction.response.send_message(f"❌ Ошибка: {e}", ephemeral=True)
...
class SetTierInfoChannelModal(discord.ui.Modal, title="📢 КАНАЛ ИНФОРМАЦИИ TIR"):
    def __init__(self):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                f"✅ Канал информации TIR настроен: {channel.mention}",
                ephemeral=True
            )
        except Exception as e:
            await interaction.response.send_message(f"❌ Ошибка: {e}", ephemeral=True)
